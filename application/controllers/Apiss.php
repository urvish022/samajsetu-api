<?php
class Apiss extends MY_controller
{
    public $village_tbl = "village_setting";
    public $app_ui_setting_tbl = "app_ui_settings";
    ...
    public function view_dashboard_ui()
    {
        ...
        if (isset($_POST["member_id"])) {
            ...
            if (isset($member_id) && !empty($member_id)) {
                ...
                if ($check_member) {
                    ...
                    $member_status = 2;
                    $member_message =
                        "Your account is Inactive Please Contact to Samaj Setu Admin (+91 9825076415)";
                }
            } else {
                $member_status = 0;
                $member_message = "You are fresh User";
            }
        } else {
            $member_status = 0;
            $member_message = "You are fresh User";
        ...
        if (empty($list) && empty($slider_list)) {
            $json = json_encode([
                ...
                "member_message" => "",
                "business" => [
                    "message" => 'This functionality has been disabled for while',
                    "status" => false
                ],
                "app_version" => "2.0",
                "app_msg" => "You are using currently old version",
                "share_msg" =>
                    "Download Samaj Setu App from Google Play Store https://play.google.com/store/apps/details?id=com.techwebsoft.samajsetu",
            ]);
        } else {
            $json = json_encode([
                "status" => "success",
                ...
                "app_version" => "2.0",
                "app_msg" =>
                    "સમાજ સેતુ એપ ની નવી અપડેટ આવી છે. જે પ્લે-સ્ટોર પરથી Samaj Setu સર્ચ કરીને અપડેટ કરવાની રહેશે.",
                "share_msg" =>
                    "Download Samaj Setu App from Google Play Store https://play.google.com/store/apps/details?id=com.techwebsoft.samajsetu",
            ]);
        }
        ...
        if (
            ...
            if ($check_member) {
                $response = json_encode([
                    "status" => "fail",
                    "status_code" => "2",
                    "eng_message" => "This email address is alredy registered",
                    "guj_message" => "આ ઇમેઇલ એડ્રેસ પહેલેથી રજીસ્ટર છે",
                ]);
            ...
            if ($check_member) {
                ...
                if ($insert_status > 0) {
                    $response = json_encode([
                        ...
                        "status_code" => "1",
                        "eng_message" =>
                            "Your registration is done successfully. Admin will verify your profile, and Username and Password will be send on your email, and that username and password will be one for all your family's mobile devices.",
                        "guj_message" =>
                            "તમારું રજીસ્ટ્રેશન થઇ ગયું છે. એડમીન ચકાસી ને એપ્રુવ કરશે ત્યારે યુઝરનામ અને પાસવર્ડ તમારા ઇમેઇલ પર મેઈલ કરવામા આવશે.અને તે યુઝરનામ અને પાસવર્ડ એક જ રહેશે પરિવાર ના બધા મોબાઈલ માટે. ધન્યવાદ.",
                    ]);
                } else {
                    $response = json_encode([
                        "status" => "fail",
                        "status_code" => "0",
                        "eng_message" => "Your profile is not added successfully.",
            ...
            $response = json_encode([
                "status" => "fail",
                "status_code" => "0",
                "eng_message" => "Please enter all required fields. Also, Email address field is required",
                "guj_message" => "કૃપા કરીને તમામ જરૂરી ફીલ્ડ્સ દાખલ કરો. ઉપરાંત, ઈમેલ એડ્રેસ ફીલ્ડ જરૂરી છે.",
            ]);
        }
            ...
            if ($check_member) {
                ...
                if ($insert_status > 0) {
                    $response = json_encode([
                        ...
                        "status_code" => "1",
                        "eng_message" =>
                            "Besna detail added successfully. Please call on samajsetu mobile number - 98250 76415",
                        "guj_message" =>
                            "આપશ્રી એ એડ કરેલ બેસણાં ની માહિતી સમાજ સેતુ માં પોસ્ટ કરવા 98250 76415 નંબર ઉપર ફોન કરી હાલ જ જણાવશો.",
                    ]);
                } else {
                    $response = json_encode([
                        "status" => "fail",
        ...
        if (isset($village_id)) {
            ...
            if ($count > 0) {
                ...
                $mydata = [
                    ...
                    "village_photo" => VILLAGE_PHOTO . $row->village_photo,
        ...
        if (empty($village_id)) {
            ...
            if ($count > 0) {
                ...
                if ($page <= $total_page) {
                    ...
                    //$list = $this->sb->get_list($this->member_details_tbl,$where_array,$columns,$count,'','full_name_eng','ASC');
                    $response = json_encode([
                        "status" => "success",
                        "status_code" => "1",
                        "data" => $list,
    ...
    public function filter_matrimony()
    {
        extract($_POST);
        $where = ["approval_flag" => 1, "met_active_flag" => 1];
        $where_string = "where approval_flag = 1 AND met_active_flag = 1";
        if (isset($full_name_eng) && !empty($full_name_eng)) {
            $fwhere = 'AND full_name_eng LIKE "%' . $full_name_eng . '%" ';
        } else {
            $fwhere = "";
        }
        if (isset($maritual_status) && !empty($maritual_status)) {
            $swhere = 'AND maritual_status = "' . $maritual_status . '" ';
        } else {
            $swhere = "";
        }
        if (isset($gender) && !empty($gender)) {
            $twhere = 'AND gender = "' . $gender . '" ';
        } else {
            $twhere = "";
        }
        if (!isset($page)) {
        ...
        $custom_qry =
            "select matrimony_id,full_name_eng,full_name_guj,village_id,photo from matrimony_details " .
            $where_string .
            " " .
            $fwhere .
            " " .
        ...
        if ($rows > 0) {
            ...
            $fetch_qry =
                ...
                " " .
                $twhere .
                " ORDER BY matrimony_id DESC LIMIT " .
                $limit .
                " OFFSET " .
                $offset .
                "";
            if ($page <= $total_page) {
                $datas = $this->sb->execute_my_query($fetch_qry);
                for ($i = 0; $i < count($datas); $i++) {
                    $list["matrimony_id"] = $datas[$i]["matrimony_id"];
                    $list["full_name_eng"] = $datas[$i]["full_name_eng"];
                    $list["full_name_guj"] = $datas[$i]["full_name_guj"];
                    $list["photo"] = $datas[$i]["photo"];
                    $vary = $this->get_village($datas[$i]["village_id"]);
                    $list["eng_name"] = $vary->eng_name;
                    $list["guj_name"] = $vary->guj_name;
                    $mylist[] = $list;
                }
                $response = json_encode([
                    "status" => "success",
    ...
    public function matrimony_list()
    {
        ...
        if ($count > 0) {
            ...
            if ($page <= $total_page) {
                ...
                for ($i = 0; $i < count($list); $i++) {
                    $datas["matrimony_id"] = $list[$i]["matrimony_id"];
                    $datas["full_name_eng"] = $list[$i]["full_name_eng"];
                    $datas["full_name_guj"] = $list[$i]["full_name_guj"];
                    $datas["guj_name"] = $list[$i]["guj_name"];
                    $datas["photo"] = $list[$i]["photo"];
                    $datas["eng_name"] = $list[$i]["eng_name"];
                    $from = new DateTime($list[$i]["birth_date"]);
    ...
    public function matrimony_single()
    {
        ...
        if (isset($matrimony_id) && !empty($matrimony_id)) {
            ...
            $list = $this->sb->get_single($this->matrimony_tbl, $where);
            $datas["matrimony_id"] = $list->matrimony_id;
            $datas["full_name_eng"] = $list->full_name_eng;
            $datas["full_name_guj"] = $list->full_name_guj;
            $vary = $this->get_village($list->village_id);
            $datas["village_id"] = $list->village_id;
            $datas["eng_name"] = $vary->eng_name;
            $datas["guj_name"] = $vary->guj_name;
            $datas["sakh"] = $list->sakh;
            $datas["birth_date"] = sqltodateformat($list->birth_date);
            $from = new DateTime($list->birth_date);
            $to = new DateTime("today");
            $age = $from->diff($to)->y;
            $datas["age"] = $age;
            $datas["gender"] = $list->gender;
            $datas["member_id"] = $list->member_id;
            $datas["education"] = $list->education;
            $datas["height"] = $list->height;
            $datas["weight"] = $list->weight;
            $datas["mosad"] = $list->mosad;
            $datas["occupation"] = $list->occupation;
            $datas["address"] = $list->address;
            $cary = $this->get_country($list->country_id);
            $datas["country_name"] = $cary->country_name;
            $datas["photo"] = $list->photo;
            $datas["maritual_status"] = $list->maritual_status;
            $datas["extra_details"] = $list->extra_details;
            if (!empty($list->mobile_no)) {
                $datas["mobile_no"] = $cary->country_std . $list->mobile_no;
            } else {
                $datas["mobile_no"] = "";
            }
            $datas["land"] = $list->land;
            $datas["mother_name"] = $list->mother_name;
            $datas["mosad_sakh"] = $list->mosad_sakh;
            $datas["father_occupation"] = $list->father_occupation;
            $datas["mother_occupation"] = $list->mother_occupation;
            $datas["brother"] = $list->brother;
            $datas["sister"] = $list->sister;
            $datas["income"] = $list->income;
            if (!empty($list->father_mobile)) {
                $datas["father_mobile"] =
                    $cary->country_std . $list->father_mobile;
            } else {
                $datas["father_mobile"] = "";
            }
            $response = json_encode([
        ...
        if ($count > 0) {
            if ($page <= $total_page) {
                ...
                for ($i = 0; $i < count($list); $i++) {
                    ...
                    $row["memorial_date"] = sqltodateformat(
                        $list[$i]["memorial_date"]
                    );
                    $row["dead_date"] = sqltodateformat($list[$i]["dead_date"]);
    ...
    public function memorial_detail()
    {
        extract($_POST);
        if (isset($memorial_id) && !empty($memorial_id)) {
            ...
            $row = $this->sb->get_single($this->memorial_tbl, $where, $columns);
            $data["memorial_id"] = $row->memorial_id;
            $data["full_name_guj"] = $row->full_name_guj;
            $data["full_name_eng"] = $row->full_name_eng;
            $data["dead_date"] = sqltodateformat($row->dead_date);
            $data["memorial_date"] = sqltodateformat($row->memorial_date);
            $vary = $this->get_village($row->village_id);
            $data["eng_name"] = $vary->eng_name;
            $data["guj_name"] = $vary->guj_name;
            $data["sakh"] = $row->sakh;
            $data["photo"] = MEMORIAL_PHOTO . $row->photo;
            $data["memorial_fulltext"] = $row->memorial_fulltext;
            $response = json_encode([
                "status" => "success",
                "status_code" => "1",
                "data" => [$data],
        ...
        if (isset($member_id)) {
            if (!empty($member_id)) {
                ...
                if ($count > 0) {
                    ...
                    if (!empty($list->mobile_no)) {
                        $member_mobile_no =
                            $country_ary->country_std . $list->mobile_no;
                    } else {
                        $member_mobile_no = "";
                    }
                    if (!empty($list->wp_mobile_no)) {
                        $member_wp_mobile_no =
                            $country_ary->country_std . $list->wp_mobile_no;
                    } else {
                        $member_wp_mobile_no = "";
                    }
                    ...
                    $member["member_id"] = $list->member_id;
                    $member["full_name_eng"] = $list->full_name_eng;
                    $member["full_name_guj"] = $list->full_name_guj;
                    ...
                    $member["mobile_no"] = $member_mobile_no;
                    $member["wp_mobile_no"] = $member_wp_mobile_no;
                    ...
                    for ($i = 0; $i < count($list2); $i++) {
                        ...
                        if (!empty($list2[$i]["fmobile_no"])) {
                            $family_list["fmobile_no"] =
                                $list2[$i]["country_std"] .
                                $list2[$i]["fmobile_no"];
                        } else {
                            $family_list["fmobile_no"] = "";
    ...
    {
        $count = $this->sb->count_all("blooddonation_details");
        if ($count > 0) {
            $data = $this->sb->get_list_nowhere(
                "blooddonation_details",
                "",
                "15",
                "",
                "bd_id",
            ...
            for ($i = 0; $i < count($data); $i++) {
                $row["bd_id"] = $data[$i]["bd_id"];
                $row["blood_grp"] =
                    $data[$i]["blood_grp"] . " blood urgently required";
                $row["req_date"] = sqltodateformat($data[$i]["req_date"]);
                $row["place"] = $data[$i]["place"];
                $datas[] = $row;
            }
            $response = json_encode([
                "status" => "success",
    ...
    public function blood_detail()
    {
        extract($_POST);
        if (isset($bd_id) && !empty($bd_id)) {
            $where = ["bd_id" => $bd_id];
            $row = $this->sb->get_single("blooddonation_details", $where);
            $list["bd_id"] = $row->bd_id;
            $list["blood_grp"] = $row->blood_grp . " blood urgently required";
            $list["req_date"] = sqltodateformat($row->req_date);
            $list["place"] = $row->place;
            $list["description"] = $row->description;
            $list["photo"] = "blood_donation.jpg";
            $single_row[] = $list;
            $response = json_encode([
                "status" => "success",
                "status_code" => "1",
    ...
    public function carrer_details()
    {
        ...
        if (isset($cpost_id)) {
            if (!empty($cpost_id)) {
                ...
                $data["cpost_id"] = $row->cpost_id;
                $data["cpost_desc"] = $row->cpost_desc;
                $desc = htmlspecialchars_decode($row->cpost_desc);
                $data["post_share_content"] = preg_replace(
                    "/<img[^>]+\>/i",
                    "",
                    $desc
                );
                $data["cpost_title"] = $row->cpost_title;
                $data["cpost_photo"] = CARRER_PHOTO . $row->cpost_photo;
                $data["cpost_date"] = sqltodateformat($row->cpost_date);
                if (!empty($row->cpost_attachment)) {
                    $atchments = json_decode($row->cpost_attachment, true);
                    for ($i = 0; $i < count($atchments); $i++) {
                        $j = $i + 1;
                        $post_attachment[$i]["cid"] = $atchments[$i];
                        $data["cpost_attachment"] = $post_attachment;
                    }
                } else {
        ...
        if (isset($news_id)) {
            if (!empty($news_id)) {
                ...
                if (!empty($row->post_attachment)) {
                    $atchments = json_decode($row->post_attachment, true);
                    for ($i = 0; $i < count($atchments); $i++) {
                        $post_attachment[$i]["aid"] = $atchments[$i];
                    }
                } else {
                    $post_attachment = [];
                }
                $desc = htmlspecialchars_decode($row->post_content);
                $new_desc = preg_replace("/<img[^>]+\>/i", "", $desc);
                $ary = [
                    "news_id" => $row->news_id,
                    "post_title" => $row->post_title,
                    "post_date" => sqltodateformat($row->post_date),
                    "post_content" => $row->post_content,
                    "post_share_content" => $new_desc,
                    "post_photo" => NEWS_PHOTO . $row->post_photo,
    ...
    public function business_detail()
    {
        ...
        if (isset($business_id)) {
            if (!empty($business_id)) {
                ...
                if (!empty($row->wp_mobile_no)) {
                    $wp_mobile_no =
                        $country_row->country_std . $row->wp_mobile_no;
                } else {
                    $wp_mobile_no = "";
                }
                $single_data = [
    ...
    public function event_detail()
    {
        ...
        if (isset($event_id) && !empty($event_id)) {
            ...
            $row = $this->sb->get_single($this->event_tbl, $where);
            $data["event_id"] = $row->event_id;
            $data["event_title"] = $row->event_title;
            $data["event_photo"] = EVENT_PHOTO . $row->event_photo;
            $data["event_content"] = $row->event_content;
            $data["event_date"] = sqltodateformat($row->event_date);
            $data["event_time"] = $row->event_time;
            $data["event_place"] = $row->event_place;
            $desc = htmlspecialchars_decode($row->event_content);
            $data["post_share_content"] = preg_replace(
                "/<img[^>]+\>/i",
                "",
                $desc
            );
            if (!empty($row->event_attachment)) {
                $ary = json_decode($row->event_attachment, true);
                for ($i = 0; $i < count($ary); $i++) {
                    $atch[$i]["aid"] = $ary[$i];
                    $data["event_attachment"] = $atch;
                }
            } else {
    ...
    public function paytm_transaction_insert()
    {
        ...
        if ($status > 0) {
            ...
            $response = json_encode(["status" => "success", "status_code" => "1"]);
        } else {
            $response = json_encode(["status" => "fail", "status_code" => "0"]);
        }
    ...
    public function proud_list()
    {
        ...
        if ($count > 0) {
            ...
            $limit = 15;
            $total_page = ceil($count / $limit);
            $colmns = ["pr_id", "full_name", "eng_name", "photo_list"];
            if ($page <= $total_page) {
                $offset = ($page - 1) * $limit;
                $join_village_tbl = [
                    "village_setting vs",
    ...
    public function proud_detail()
    {
        extract($_POST);
        if (isset($pr_id) && !empty($pr_id)) {
            $where = ["pr_id" => $pr_id];
            $list = $this->sb->get_single($this->proud_tbl, $where);
            $datas["pr_id"] = $list->pr_id;
            $datas["full_name"] = $list->full_name;
            $village_ary = $this->get_village($list->village_id);
            $datas["eng_name"] = $village_ary->eng_name;
            if (!empty($list->photo_list)) {
                $photo_lists = json_decode($list->photo_list, true);
                for ($i = 0; $i < count($photo_lists); $i++) {
                    $plist[$i]["pid"] = $photo_lists[$i];
                    $datas["photo_list"] = $plist;
                }
            } else {
                $datas["photo_list"] = [];
            }
            if (!empty($list->post_attachment)) {
                $photo_atchs = json_decode($list->post_attachment, true);
                for ($i = 0; $i < count($photo_atchs); $i++) {
                    $alist[$i]["aid"] = $photo_atchs[$i];
                    $datas["post_attachment"] = $alist;
                }
            } else {
                $datas["post_attachment"] = [];
            }
            if (!empty($list->photo_list)) {
                $datas["photo"] = PROUD_PHOTO . $photo_lists[0];
            } else {
                $datas["photo"] = "";
            }
            $desc = htmlspecialchars_decode($list->proud_details);
            $datas["post_share_content"] = preg_replace(
                "/<img[^>]+\>/i",
                "",
                $desc
            );
            $datas["proud_details"] = $list->proud_details;
            $data[] = $datas;
            $response = json_encode([
                "status" => "success",
                "status_code" => "1",
        ...
        if ($count > 0) {
            ...
            if ($page <= $total_page) {
                for ($i = 0; $i < count($list); $i++) {
                    ...
                    $desc = htmlspecialchars_decode($list[$i]["org_details"]);
                    $row["post_share_content"] = preg_replace(
                        "/<img[^>]+\>/i",
                        "",
                        $desc
                    );
            ...
            if (
                ...
                if ($rows > 0) {
                    ...
                    if (
                        ...
                        $photo_ary = $this->sb->get_single(
                            $this->member_details_tbl,
                            $where,
                            ["photo"]
                        );
                        $photo = $photo_ary->photo;
                    }
                    $status = $this->sb->update(
                        $this->member_details_tbl,
                        $member_array,
                    ...
                    if ($status > 0) {
                        ...
                        $response = json_encode([
                            "status" => "fail",
                            "status_code" => "0",
                            "eng_message" => "Error! Update profile failed!",
                ...
                $response = json_encode([
                    "status" => "fail",
                    "status_code" => "0",
                    "eng_message" => "Required fields are missing",
                    "guj_message" => "કંઈક ખોટું થઇ ગયું છે",
                ]);
            }
        } catch(\Exception $e){
    ...
    public function business_renew()
    {
        extract($_POST);
        if (!empty($member_id)) {
            $order_no = $this->sb->get_custom_id(
            ...
            $row = $this->sb->get_single("business_directory", $where, "");
            $member_array = [
                "bc_id" => $row->bc_id,
                "owner_name" => $row->owner_name,
                "company_name_eng" => ucwords($row->company_name_eng),
                "company_name_guj" => $row->company_name_guj,
                "village_id" => $row->village_id,
                "member_id" => $row->member_id,
                ...
                "website" => $row->website,
                "photo" => $row->photo,
                "timing" => ucwords($row->timing),
                "details" => ucwords($row->details),
                "ad_category" => "business_ad",
                "ad_duration_year" => $duration,
                "order_no" => $order_no,
                "payment_status" => "PENDING",
            ];
            $insert_status = $this->sb->insert(
            ...
            if ($insert_status > 0) {
                ...
                if (!empty($checksumhash["checksum"])) {
                    $res = json_encode([
                        ...
                        "order_no" => $order_no,
                        "MID" => $checksumhash["paramList"]["MID"],
                        "ORDER_ID" => $checksumhash["paramList"]["ORDER_ID"],
                        "CUST_ID" => $checksumhash["paramList"]["CUST_ID"],
                        "CHANNEL_ID" =>
                            $checksumhash["paramList"]["CHANNEL_ID"],
                        "TXN_AMOUNT" =>
            ...
            if (!empty($member_id)) {
                ...
                if ($count > 0) {
                    ...
                    for ($i = 0; $i < count($rows); $i++) {
                        ...
                        if (
                            $rows[$i]["ad_start_date"] != null &&
                            $rows[$i]["ad_end_date"] != null
                        ) {
                            $date1_ts = strtotime(date("Y-m-d"));
                            $date2_ts = strtotime($rows[$i]["ad_end_date"]);
                        ...
                        $data = [
                            ...
                            "ad_start_date" => sqltodateformat(
                                $rows[$i]["ad_start_date"]
                            ),
                            "ad_end_date" => sqltodateformat(
                                $rows[$i]["ad_end_date"]
                            ),
                            "remain_days" => $remain,
                        ];
        ...
        if ($count > 0) {
            ...
            if ($page <= $total_page) {
                ...
                for ($i = 0; $i < count($list); $i++) {
                    ...
                    $row["TXNDATE"] = sqltodateformat($list[$i]["TXNDATE"]);
                    $row["DETAILS"] = $list[$i]["DETAILS"];
                    $row["TOTALAMT"] = $list[$i]["TOTALAMT"];
                    $row["TYPE"] = $list[$i]["TYPE"];
                    $mydata[] = $row;
                }
                //$list['TXNDATE'] = sqltodateformat($list['TXNDATE']);
                $res = json_encode([
                    "status" => "success",
                    "status_code" => "1",
                    "data" => $mydata,
    ...
    public function generate_otp()
    {
        ...
        if (!empty($user_name)) {
            ...
            $member = $this->sb->get_single(
                $this->member_details_tbl,
                $where
            );
            if (!empty($member)) {
                if($member->approval_flag == 1 && $member->mem_active_flag == 1){
                    // call_app_curl($member->member_id, MAIL_API_URL);
                    $password = my_simple_crypt($member->password,'d');
                    $this->sendMail($member->user_name,$member->email_id,$password,$member->full_name_eng,"reset");
                    $res = json_encode([
                        "status" => "true",
                        "status_code" => "1",
                        "eng_message" => "We have sent a password to your registered email. Please check your inbox."
                    ]);
                } else {
                    $res = json_encode([
                        "status" => "fail",
                ...
                $res = json_encode([
                    "status" => "fail",
                    "status_code" => "0",
                    "eng_message" => "Your email id is not registered.",
                ]);
            }
        } else {
            $res = json_encode([
                "status" => "fail",
                "status_code" => "0",
                "eng_message" => "Please enter valid email.",
            ]);
        }
    ...
    public function save_fcm_token()
    {
        ...
        if (
            ...
            $fcm_check = $this->sb->data_exists($this->fcm_tbl, $fcm_ary);
            if ($fcm_check) {
                $update_ary = ["fcm_token" => $fcm_token];
                $this->sb->update(
                    $this->fcm_tbl,
                ...
                $this->sb->insert($this->fcm_tbl, $insert_ary);
            }
            $res = json_encode(["status" => "success", "status_code" => "1"]);
        } else {
            $res = json_encode(["status" => "fail", "status_code" => "0"]);
        }
    ...
    public function upload_curl_image()
    {    
        ...
        if (
            ...
            $file = $_FILES["photo"]["name"];
            $upload = $this->upload_existing_img(
                $file,
                "5000",
                "100%",
                "300",

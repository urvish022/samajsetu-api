...
class Pending_slider extends MY_Controller
{
  ...
  public function index()
  {
    ...
    $this->layout->view("pending_slider");
  }
  ...
  public function approve_business()
  {
    ...
    if (!empty($business_id)) {
      ...
      if ($response > 0) {
        $where_slider = ["set_as_active" => "1"];
        $row = $this->sb->get_single(
          "slider_payment_setting",
          $where_slider,
          "sp_id"
        );
        $slider_name = "slider" . $row->sp_id;
        $updatewhere = ["slider_name" => $slider_name];
        $expire_date = date("Y-m-d", strtotime("+1 years"));
        $setdata = [
          "slider_photo" => $photo,
          "slider_startdate" => date("Y-m-d"),
          "slider_enddate" => $expire_date,
          "business_id" => $business_id,
        ];
        $this->sb->update("slider_setting", $setdata, $updatewhere);
        copy(
          ASSET_BUSINESS_PHOTO . $old_photo,
          ASSET_SLIDER_PHOTO . $photo
        );
        echo json_encode([
          "update_status" => true,
          "id" => $business_id,
  ...
  public function send_fcm_notify()
  {
    ...
    $notificationdata = [
      ...
      "notification_id" => $id,
      "model_id" => "App\Models\SliderSetting",
    ];
